...
ALL projects route STATUS.md and KNOWLEDGE.md to `.build/` regardless of collaborator status. The project CLAUDE.md (which stays at root) references these via relative path `.build/STATUS.md` etc. so future sessions can find them without committing transient working-state.
...
If the project has no STATUS.md (in any location), skip 4.1. If it uses a different living doc (e.g. `LIFE_CONTEXT.md`, `INBvendor.md`), update that instead and note the substitution in your final report.
...
**Universal `.build/cache/sessions/` rule:** ALL projects route session logs to `.build/cache/sessions/` regardless of collaborator status. Session logs are local working-state (draft decisions, work journals, transient notes) and should stay out of tracked project history unless explicitly promoted into durable docs. `SESSION_INDEX.md` lives alongside them at `.build/cache/sessions/SESSION_INDEX.md`.

**Project-specific exceptions:** If a nested subproject should share the parent project's `.build/`, document that exception in the project `AGENTS.md` before relying on it.
